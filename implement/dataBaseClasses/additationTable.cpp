...
AdditationTable::AdditationTable(sql::Connection* conn):connection(conn){};
    ...
    bool AdditationTable::addAdditationTransaction(nlohmann::json json){
        int expense = 0;
        std::string date;       
        std::string reasone;
    ...
    bool AdditationTable::updateAdditationTransaction(nlohmann::json json){
        int id = 0;
        std::string date;
        int expense = 0;
        std::string reasone;
        ...
        if(json.contains("id")){
            ...
            std::cout << "updateAdditationTransaction id not found" << std::endl;
            return false;
        }
        if(json.contains("date")){
            date = json["date"];
        }else{
            std::cout << "updateAdditationTransaction date not found" << std::endl;
            return false;
        }
        if(json.contains("expense")){
            expense = json["expense"];
        }else{
            std::cout << "updateAdditationTransaction expense not found" << std::endl;
            return false;
        }
    ...
    bool AdditationTable::deleteAdditationTransaction(nlohmann::json json){
        int id;
        if(json.contains("id")){
            id = json["id"];
        }else{
    ...
    nlohmann::json AdditationTable::getAdditationTransactionsByInterval(nlohmann::json json){
    ...
    int AdditationTable::getAdditationTransactionsTotalByInterval(nlohmann::json json){
        std::string startDate;
        std::string endDate;
